...
void
Bat::Behavior(bool isPlayerAttacking, Camera* camera,
	Image* map, Inventory* inventory)
{
	...
	if ((currentTime - this->lastDirChange) > 2) {
		this->moveFactor = (float)GetRandomValue(1, 3);
		this->lastDirChange = currentTime;
	}

	this->velocity = (Vector3){
		cosf(GetTime() * this->moveFactor) * 1.f * dt,
		0.f,
		sinf(GetTime() * this->moveFactor) * 1.f * dt
	};
}

void
Bat::CheckCollisions(Image* map)
{
